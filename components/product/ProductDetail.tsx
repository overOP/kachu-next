        ...
        <section className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 rounded-3xl bg-white dark:bg-zinc-900 p-4 sm:p-8 md:p-10 border border-emerald-100 dark:border-zinc-800 shadow-2xl shadow-emerald-900/5 dark:shadow-black/50 dark:ring-1 dark:ring-zinc-800">
          <div className="relative aspect-square overflow-hidden rounded-2xl bg-slate-50 dark:bg-zinc-800/70">
            <Image
              ...
              priority
            />
            <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-white dark:bg-zinc-900 shadow-sm p-2 rounded-xl">
              <div className="relative h-8 w-8">
                <Image src={product.logo} alt={product.brand} fill className="object-contain" />
              </div>
            </div>
          </div>

          <div className="flex flex-col justify-center">
            <p className="text-xs uppercase tracking-[0.25em] font-bold text-emerald-600 dark:text-sky-400">{product.brand}</p>
            <h1 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-emerald-950 dark:text-zinc-50">
              {product.name}
            </h1>
            <p className="mt-4 text-sm sm:text-base text-slate-600 dark:text-zinc-400 leading-relaxed">{product.Description}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {relatedProducts.map((related) => (
                    <Link
                      ...
                      className="group flex items-center gap-3 rounded-xl border border-emerald-100 dark:border-zinc-700 bg-emerald-50/40 dark:bg-zinc-800/60 p-2.5 hover:border-emerald-300 dark:hover:border-sky-600/50 hover:bg-emerald-50 dark:hover:bg-zinc-800 transition-colors"
                    >
                      <div className="relative h-14 w-14 rounded-lg bg-white dark:bg-zinc-900 overflow-hidden shrink-0">
                        <Image src={related.img} alt={related.name} fill className="object-contain p-2" />
                      </div>
